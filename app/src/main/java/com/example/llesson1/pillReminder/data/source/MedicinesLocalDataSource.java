...
public class MedicinesLocalDataSource implements MedicineDataSource {
    ...
    private static com.example.llesson1.pillReminder.data.source.MedicinesLocalDataSource mInstance;

    private AlarmReminderDBHelper mDbHelper;


    private MedicinesLocalDataSource(Context context) {
        mDbHelper = new AlarmReminderDBHelper(context);
    }

    public static com.example.llesson1.pillReminder.data.source.MedicinesLocalDataSource getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new com.example.llesson1.pillReminder.data.source.MedicinesLocalDataSource(context);
        }
        return mInstance;
    }
